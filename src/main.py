"""GoldBot main entry point — connects MT5, runs strategy engine on a schedule."""
...
class GoldBot:
    ...
    def __init__(self, config_path: str = "config/strategies.yaml", env_path: str = ".env"):
        ...
        self.comment_prefix = self.config["general"].get("comment_prefix", "GB_")
        ...
        self.notifier = TelegramNotifier(env_path)
    ...
    def start(self):
        ...
        # Schedule daily summary at 21:00 UTC (after hard exit)
        self.scheduler.add_job(
            self._daily_summary,
            "cron",
            hour=21, minute=0,
            ...
            if result and result.get("type") == "trade_opened":
                ...
                self.notifier.trade_opened(
                    ...
                    trade={
                        ...
                        "comment": trade_result.get("comment", "ITF"),
                    },
                    risk_pct=result["risk_pct"],
                    regime=result["regime"],
                )

            # Update live P&L on open positions (edits Telegram message)
            self._update_open_positions()

            # Check for newly closed positions (compare with DB)
    ...
    def _update_open_positions(self):
        ...
        for pos in mt5_positions:
            if not pos.get("comment", "").startswith(self.comment_prefix):
                continue
            # Find matching DB record for equity_at_entry
            db_trade = next(
            ...
            self.notifier.trade_updated(
                ...
                trade={
                    ...
                    "sl": pos["sl"],
                    "tp": pos["tp"],
                    "lots": pos["volume"] if "volume" in pos else pos.get("lots", 0),
                    "profit": pos["profit"],
    ...
    def _check_closed_positions(self):
        ...
        for trade in db_open:
            if trade["ticket"] not in mt5_open_tickets:
                ...
                # Send/edit Telegram closure message
                self.notifier.trade_closed(
                    ...
                    trade={
                        ...
                        "strategy": trade.get("strategy", "ITF"),
                    },
                )
    ...
    def _setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                ...
                logging.FileHandler("data/goldbot.log", encoding="utf-8"),
            ],
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="GoldBot — Automated XAU/USD Trading")
    parser.add_argument("--config", default="config/strategies.yaml", help="Path to config file")
    parser.add_argument("--env", default=".env", help="Path to .env file")
    args = parser.parse_args()

    bot = GoldBot(config_path=args.config, env_path=args.env)
    bot.start()
