...
- While trade is LIVE: edits the same message to update P&L and trailing SL
- On trade CLOSE: edits the same message to show final summary
- One message per trade, updated in-place
"""
...
class TelegramNotifier:
    """Sends and edits formatted alerts to Telegram."""
    ...
    def __init__(self, env_path: str = ".env"):
        ...
        # Track message IDs per ticket for editing
        self._trade_messages: dict[int, int] = {}  # ticket -> telegram message_id
    ...
    def trade_opened(self, ticket: int, trade: dict, risk_pct: float, regime: str):
        """Send initial trade message when a position is opened."""
        direction = trade["direction"].upper()
        emoji = "\U0001f7e2" if trade["direction"] == "buy" else "\U0001f534"

        msg = (
            f"{emoji} <b>TRADE OPEN — {direction} XAUUSDm</b>\n"
            f"\n"
            ...
            f"Status: <b>OPEN</b>\n"
            f"\n"
            f"Strategy: {trade.get('comment', 'ITF')}\n"
            f"Regime: {regime}\n"
        ...
        if message_id:
            ...
            logger.info("Telegram trade message sent: ticket=%s, msg_id=%s", ticket, message_id)

    def trade_updated(self, ticket: int, trade: dict):
        """Edit the trade message with current P&L and trailing SL.

        If no message is tracked (e.g. after bot restart), sends a new message
        and starts tracking it so future updates edit in-place.
        """
        direction = trade["direction"].upper()
        emoji = "\U0001f7e2" if trade["direction"] == "buy" else "\U0001f534"
        ...
        if message_id:
            ...
            # No tracked message (bot restarted) — send a new one and start tracking
            new_id = self._send_sync(msg)
            if new_id:
                self._trade_messages[ticket] = new_id
                logger.info("Telegram: sent new message for existing trade ticket=%s, msg_id=%s", ticket, new_id)

    def trade_closed(self, ticket: int, trade: dict):
        """Edit the trade message to show final closure details."""
        message_id = self._trade_messages.get(ticket)
        ...
        msg = (
            f"{result_emoji} <b>TRADE CLOSED — {direction} XAUUSDm</b>\n"
            f"\n"
            f"Entry:  <code>{entry:.2f}</code>\n"
            ...
            f"Status: <b>CLOSED</b>\n"
            f"\n"
            f"Strategy: {trade.get('strategy', 'ITF')}\n"
    ...
    # ── System alerts (unchanged — separate messages) ────────────────────────
    ...
    def alert_circuit_breaker(self, reason: str, daily_pnl_pct: float):
        msg = (
            f"\U000026a0\U0000fe0f <b>CIRCUIT BREAKER TRIGGERED</b>\n"
            f"Reason: {reason}\n"
    ...
    def alert_system_start(self, mode: str, equity: float, strategies: list[str]):
        msg = (
            f"\U0001f680 <b>GoldBot Started</b>\n"
    ...
    def alert_system_stop(self, reason: str = "Manual"):
        msg = (
            f"\U0001f6d1 <b>GoldBot Stopped</b>\n"
            f"Reason: {reason}\n"
            f"Time: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}"
        )
        self._send_sync(msg)
    ...
    def alert_connection_lost(self):
        msg = (
            f"\U000026a0\U0000fe0f <b>MT5 CONNECTION LOST</b>\n"
