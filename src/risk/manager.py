...
class RiskManager:
    ...
    def __init__(self, config_path: str = "config/strategies.yaml"):
        ...
        self._comment_prefix = "GB_"
        self._magic_number = 123456

        # Track daily P&L as % of starting equity
    ...
    def update_equity_snapshot(self, current_equity: float):
        """Call at the start of each trading day and on each trade close."""
    ...
    def get_daily_pnl_pct(self, current_equity: float) -> float:
        """Return today's P&L as percentage of day-start equity."""
    ...
    def get_drawdown_from_peak_pct(self, current_equity: float) -> float:
        """Return drawdown from peak equity as a positive percentage."""
    ...
    def check_circuit_breakers(self, current_equity: float) -> dict:
        """Check all circuit breaker conditions. Returns status dict."""
        ...
        # Monthly drawdown alert — reduce risk
        if drawdown_pct >= self.monthly_drawdown_alert_pct:
            status["monthly_alert"] = True
            status["risk_per_trade_pct"] = 1.0  # halve the risk
            logger.warning(
                "Drawdown alert: %.2f%% — reducing risk to 1%% per trade",
                drawdown_pct,
            )
    ...
    def calculate_lot_size(
        ...
        Args:
            ...
            symbol_info: Dict from MarketFeed.get_symbol_info()
            risk_override_pct: Override risk % (e.g., during drawdown alert)

        Returns:
            Lot size rounded to volume_step
        """
        ...
        # Value per price unit per lot
        value_per_point = tick_value / tick_size

        # How much 1 lot would lose at the SL distance
    ...
    def get_effective_risk_pct(self, current_equity: float) -> float:
        """Get the current risk % accounting for drawdown reductions."""
        status = self.check_circuit_breakers(current_equity)
...
if __name__ == "__main__":
    ...
    # Simulate with $500 account
    equity = 500.0
    rm.update_equity_snapshot(equity)

    # Example: XAUUSD SL distance of $8 (about 1.65 × ATR)
    mock_symbol_info = {
        "trade_tick_value": 1.0,  # $1 per tick per lot
    ...
    lots = rm.calculate_lot_size(equity, sl_distance_price=8.0, symbol_info=mock_symbol_info)
    print(f"\n$500 account, $8 SL distance → {lots} lots")
    print(f"Risk amount: ${equity * rm.risk_per_trade_pct / 100:.2f} ({rm.risk_per_trade_pct}%)")

    # Check circuit breaker with $485 equity (3% loss)
    status = rm.check_circuit_breakers(485.0)
    print(f"\nCircuit breaker check at $485: {status}")
