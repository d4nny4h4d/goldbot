...
class Database:
    """SQLite-backed trade journal. Stores both $ and % values. Thread-safe."""

    def __init__(self, db_path: str = "data/goldbot.db"):
    ...
    def _execute(self, query: str, params: tuple = ()):
        """Thread-safe execute with commit."""
        with self._lock:
            self._conn.execute(query, params)
            self._conn.commit()

    def _query(self, query: str, params: tuple = ()) -> list:
        """Thread-safe query returning list of Rows."""
        with self._lock:
            return self._conn.execute(query, params).fetchall()

    def _query_one(self, query: str, params: tuple = ()):
        """Thread-safe query returning single Row."""
        with self._lock:
    ...
    def log_trade_open(
        ...
        self._execute(
            ...
            (
                ticket, symbol, direction, lots, entry_price, sl, tp,
                strategy, datetime.now(timezone.utc).isoformat(),
                equity_at_entry, comment,
            ),
        )
    ...
    def get_today_trades(self) -> list[dict]:
        """Get all trades opened or closed today."""
    ...
    def save_equity_snapshot(
        self, equity: float, balance: float, unrealized_pnl: float, open_positions: int
    ):
        self._execute(
            ...
            (
                ...
                equity, balance, unrealized_pnl, open_positions,
            ),
        )
    ...
    def get_performance_stats(self) -> dict:
        """Calculate overall performance metrics from closed trades."""
        rows = self._query(
            "SELECT profit_usd, profit_pct FROM trades WHERE status = 'closed'"
        )
...
if __name__ == "__main__":
    ...
    print(f"Performance stats: {db.get_performance_stats()}")
    db.close()
